...
| 红线 | 说明 |
|---|---|
| 禁止无理由的 `any` | 使用 `unknown` + 类型守卫替代 |
| 禁止 `as` 强制转换规避错误 | 修复类型本身 |
| 公共函数必须有返回类型 | 防止隐式 `any` |
| Hooks 依赖数组必须完整 | `useEffect` / `useMemo` / `useCallback` |
...
> **禁止**：修改 `.eslintrc` / `biome.json` / `tsconfig.json` 来让检查通过。只允许修复代码本身。
...
| 风险 | 要求 |
|---|---|
| XSS | 用户输入禁止直接 `dangerouslySetInnerHTML` |
| CSRF | 状态变更操作必须携带 CSRF Token 或 2FA 验证 |
| 权限 | 受保护路由必须前后端双重校验 |
...
```bash
# 安装（若项目尚未引入）
npm install --save-dev @axe-core/react   # React 集成
# 或 Vitest / Jest 环境
